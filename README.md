...
現状、`mask_coords.json` は「直近1件のマスク設定」しか保持できず、`main.py` は常にそれを `input/` 内の全PDFへ一律適用する(レイアウトA用に採寸したら、レイアウトBを処理する前に採寸し直す必要がある)。複数の帳票フォーマットを日常的に使い分けるユーザー向けに、以下を検討する。

- **テンプレート保存**: `MaskPicker` で採寸した内容を、上書きではなく名前付きの「テンプレート」として複数保存できるようにする(例: `templates/給与明細.json` のようにレイアウトごとに保存)。
- **テンプレート選択**: `PDFMaskingTool` 実行時に、処理対象のPDFに合わせて使用するテンプレートを選べるようにする(例: 起動時に一覧から選択するダイアログを出す、またはPDFのレイアウトを自動判定する等)。

いずれも `mask_coords.json` 単一ファイル・「最後に保存した1件を使う」という現在の設計を見直すことになるため、着手前にデータ形式(テンプレートの保存場所・命名規則)と選択UIの方針を決める。
